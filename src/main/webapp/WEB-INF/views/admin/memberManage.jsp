...
<section class="module">
	<div class="container">
		<div class="row">
			<div>
				<h2>
					회원 관리<small>(${count})</small>
				</h2>
				<hr class="divider-w mt-10 mb-20">
				<form class="form" role="form" method="post"
					action="/admin/memberManage/delete">
					<table class="table table-border checkout-table">
						<tbody>
							...
							<c:forEach items="${mlist}" var="m">
								<tr>
									<td><input type="checkbox" data-id="${m.id}" class="chBox"
										name="chBox" id="chBox"></td>
									<td>${m.member_id}</td>
									<td>${m.member_name}</td>
									<td>${m.member_phone}</td>
									<td>${m.email}</td>
									<td>(${m.member_addr1}) ${m.member_addr2}
										${m.member_addr3}</td>

									<c:if test="${m.enabled eq true}">
										<td>가입 회원</td>
									</c:if>

									<c:if test="${m.enabled eq false}">
										<td>탈퇴 회원</td>
									</c:if>

									<c:if test="${m.member_id != 'admin'}">
										<td><input type="button" id="memDeleteBtn"
											class="btn btn-d btn-round"
											onclick="location.href='/admin/memberManage/delete/${m.id}'"
											value="삭제"></td>
									</c:if>
					...
					<hr class="divider-w mt-10 mb-20">
					<button id="allMemDeleteBtn" class="btn btn-d btn-round"
						type="button">선택 회원 삭제</button>
				</form>
			</div>
		</div>
	</div>
				...
				<ul class="pagination">
					...
					<c:if test="${page.startPage > page.blockSize}">
						<li class="page-item"><a class="page-link"
							href="/admin/memberManage?pageNum=${page.startPage - page.blockSize}&field=${field}&word=${word}">Previous</a></li>
					</c:if>
					<!--페이지 리스트-->
					<c:forEach begin="${page.startPage}" end="${page.endPage}" var="i">
						<li class="page-item"><a class="page-link"
							href="memberManage?pageNum=${i}&field=${field}&word=${word}">${i}</a></li>
					</c:forEach>
					<!-- 다음 -->
					<c:if test="${page.endPage < page.totPage}">
						<li class="page-item"><a class="page-link"
							href="memberManage?pageNum=${page.endPage + 1}&field=${field}&word=${word}">Next</a></li>
					</c:if>
				</ul>
			</div>
			...
			<form class="form" action="/admin/memberManage" id="searchFrm">
				<select name="field" class="form">
					...
					<option value="member_name"
						${(field == 'member_name') ? 'selected' : ''}>아이디</option>
